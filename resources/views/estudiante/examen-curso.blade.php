...
@section('title', 'Examen Final del Curso | SAINS')
...
<div class="simulator-container">
        ...
        <div class="exam-header-content">
            <div class="exam-info">
                <div class="exam-badge">
                    ...
                    <span class="badge-status final-badge ms-2">
                ...
                <h1 class="exam-title" id="examenNombre">
                    ...
                    {{ $examen->tipo_examen ?? 'Examen Final del Curso' }}
                </h1>
                <div class="exam-meta">
    ...
    <div id="preguntasContainer" class="questions-container">
        @foreach($preguntas as $index => $pregunta)
        @php
            // Crear array con las 3 opciones: 1 correcta + 2 incorrectas
            $opciones = [];
            
            // Opción 1 - Correcta
            if($pregunta->respuesta_correcta && trim($pregunta->respuesta_correcta) !== '') {
                $opciones[] = [
                    'valor' => 'correcta',
            ...
            // Opción 2 - Incorrecta 1
            if($pregunta->respuesta1 && trim($pregunta->respuesta1) !== '') {
                $opciones[] = [
                    'valor' => 'incorrecta1',
            ...
            // Opción 3 - Incorrecta 2
            if($pregunta->respuesta2 && trim($pregunta->respuesta2) !== '') {
                $opciones[] = [
                    'valor' => 'incorrecta2',
            ...
            // Mezclar opciones ALEATORIAMENTE para que el orden cambie
            shuffle($opciones);
            
            // Reasignar letras después de mezclar
            $letras = ['A', 'B', 'C'];
...
<script>
    ...
    console.log('=== INICIALIZACIÓN EXAMEN FINAL DEL CURSO ===');
        ...
        timerInterval = setInterval(() => {
            ...
            if (tiempoRestanteSegundos <= 0) {
                ...
                Swal.fire({
                    ...
                    text: 'El tiempo del examen final ha terminado. Enviando respuestas...',
                    icon: 'info',
    ...
    function actualizarDisplayTimer() {
        ...
        if (tiempoRestanteSegundos === 60) {
            Swal.fire({
                ...
                text: 'Último minuto para completar tu examen final',
                icon: 'warning',
                confirmButtonColor: '#f59e0b',
                timer: 5000
            });
    ...
    function guardarRespuesta(preguntaId, valor) {
        ...
        actualizarProgreso();
        localStorage.setItem(`curso_${examenActualId}_respuestas`, JSON.stringify(respuestasRegistradas));
        localStorage.setItem(`curso_${examenActualId}_tiempo`, tiempoRestanteSegundos);
    }
    ...
    function finalizarExamen() {
        ...
        if (respondidas < totalPreguntas) {
            Swal.fire({
                ...
                confirmButtonText: 'Sí, finalizar examen final',
    ...
    function enviarExamen() {
        ...
        fetch('{{ route("estudiante.responder.examen-curso") }}', {
            method: 'POST',
            body: formData
        })
        .then(res => res.json())
        .then(data => {
            if (data.success) {
                localStorage.removeItem(`curso_${examenActualId}_respuestas`);
                localStorage.removeItem(`curso_${examenActualId}_tiempo`);
                
                Swal.fire({
                    icon: data.calificacion >= 70 ? 'success' : 'info',
                    title: data.calificacion >= 70 ? '🎉 ¡Examen Final Aprobado!' : '📚 Examen Final Completado',
                    html: `<div class="text-center">
                        <div class="display-1 fw-bold" style="color: ${data.calificacion >= 70 ? '#10b981' : '#f59e0b'};">${data.calificacion}%</div>
                        ...
                        <div class="progress mx-auto" style="height: 10px; max-width: 200px;">
                            <div class="progress-bar" style="width: ${data.calificacion}%; background: ${data.calificacion >= 70 ? '#10b981' : '#f59e0b'}"></div>
                        </div>
                        ...
                        ${data.calificacion >= 70 ? '<p class="mt-3"><strong>🏆 ¡Felicidades! Has completado exitosamente el curso</strong></p>' : '<p class="mt-3">📚 Puedes intentarlo nuevamente para mejorar tu puntuación</p>'}
                    </div>`,
        ...
        .catch(error => {
            ...
            Swal.fire('Error', 'Error al enviar el examen final', 'error');
            examStarted = true;
            iniciarTimer();
        });
    }
    ...
    document.addEventListener('DOMContentLoaded', function() {
        if (totalPreguntas > 0) {
            const saved = localStorage.getItem(`curso_${examenActualId}_respuestas`);
            if (saved) {
            ...
            const savedTime = localStorage.getItem(`curso_${examenActualId}_tiempo`);
