...
@section('title', 'Editar Interacción - Call Center SAINS')
        ...
        <div class="d-flex justify-content-between align-items-center flex-wrap gap-3">
            <div>
                <h1 class="display-5 fw-bold" style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); -webkit-background-clip: text; background-clip: text; color: transparent;">
                    <i class="fas fa-edit me-3"></i>Editar Interacción
                </h1>
                <p class="text-muted">Modifique los detalles del contacto con el estudiante</p>
            </div>
...
<div class="row">
    <div class="col-12">
        <div class="card-modern p-4 p-lg-5">
            <form action="{{ route('admin.callcenter.update', $interaccion->id) }}" method="POST" id="interaccionForm" novalidate>
                @csrf
                @method('PUT')
                    ...
                    <div class="row g-4">
                        <div class="col-md-6">
                            ...
                            <select name="id_estudiante" id="id_estudiante" class="form-select form-select-lg @error('id_estudiante') is-invalid @enderror" required>
                                ...
                                @foreach($estudiantes as $estudiante)
                                    <option value="{{ $estudiante->id }}" {{ old('id_estudiante', $interaccion->id_estudiante) == $estudiante->id ? 'selected' : '' }}>
                            ...
                            @enderror
                            <small class="text-muted">Seleccione el estudiante contactado</small>
                        </div>
                        ...
                        <div class="col-md-3">
                            ...
                            <input type="date" name="fecha_contacto" id="fecha_contacto" 
                                   ...
                                   value="{{ old('fecha_contacto', $interaccion->fecha_contacto ? $interaccion->fecha_contacto->format('Y-m-d') : date('Y-m-d')) }}" required>
                        ...
                        <div class="col-md-3">
                            ...
                            <input type="time" name="hora_contacto" id="hora_contacto" 
                                   class="form-control form-control-lg @error('hora_contacto') is-invalid @enderror" 
                                   value="{{ old('hora_contacto', $interaccion->hora_contacto ? date('H:i', strtotime($interaccion->hora_contacto)) : date('H:i')) }}" required>
                            @error('hora_contacto')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                            <small class="text-muted">Hora del contacto</small>
                        </div>
                        ...
                        <div class="col-md-6">
                            ...
                            <select name="tipo_contacto" id="tipo_contacto" class="form-select form-select-lg @error('tipo_contacto') is-invalid @enderror" required>
                                ...
                                @foreach($tiposContacto as $key => $label)
                                    <option value="{{ $key }}" {{ old('tipo_contacto', $interaccion->tipo_contacto) == $key ? 'selected' : '' }}>
                                        @if($key == 'llamada')
                        ...
                        <div class="col-md-6">
                            ...
                            <select name="estado_seguimiento" id="estado_seguimiento" class="form-select form-select-lg @error('estado_seguimiento') is-invalid @enderror" required>
                                ...
                                @foreach($estadosSeguimiento as $key => $label)
                                    <option value="{{ $key }}" {{ old('estado_seguimiento', $interaccion->estado_seguimiento) == $key ? 'selected' : '' }}>
                                        @if($key == 'pendiente')
                    ...
                    <div class="row g-4">
                        <div class="col-12">
                            ...
                            <textarea name="motivo_contacto" id="motivo_contacto" rows="3" 
                                      ...
                                      placeholder="Ej: Consulta sobre planes de estudio, seguimiento académico, información de pagos..." required>{{ old('motivo_contacto', $interaccion->motivo_contacto) }}</textarea>
                        ...
                        <div class="col-12">
                            ...
                            <textarea name="nota" id="nota" rows="4" 
                                      ...
                                      placeholder="Registre los detalles importantes de la conversación, dudas del estudiante, acuerdos, etc.">{{ old('nota', $interaccion->nota) }}</textarea>
                            @error('nota')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="col-12">
                            ...
                            <textarea name="resultado" id="resultado" rows="3" 
                                      ...
                                      placeholder="Ej: Estudiante interesado en plan básico, solicita información adicional, se envía correo con detalles...">{{ old('resultado', $interaccion->resultado) }}</textarea>
                            @error('resultado')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                    ...
                    <div class="row g-4">
                        <div class="col-md-12">
                            ...
                            <input type="datetime-local" name="proximo_contacto" id="proximo_contacto" 
                                   ...
                                   value="{{ old('proximo_contacto', $interaccion->proximo_contacto ? \Carbon\Carbon::parse($interaccion->proximo_contacto)->format('Y-m-d\TH:i') : '') }}">
                ...
                <div class="d-flex justify-content-between gap-3 mt-5 pt-4 border-top">
                    ...
                    <button type="submit" class="btn btn-save px-5 py-3 rounded-pill" id="submitBtn" style="display: none;">
                        <i class="fas fa-save me-2"></i> Actualizar Interacción
                    </button>
...
<script>
    ...
    const horaContactoInput = document.getElementById('hora_contacto');
    ...
    function mostrarInfoEstudiante() {
        ...
        if (selectedOption && selectedOption.value) {
            ...
            // Verificar si el texto contiene indicadores de plan
            const tienePlanActivo = textoCompleto.includes('Plan Activo');
            const tieneSinPlan = textoCompleto.includes('Sin Plan Activo');
            ...
            if (tienePlanActivo) {
                ...
                estadoPlan = '⚠️ Sin Plan Activo';
                colorPlan = '#ff9800';
            }
            
            infoTexto.innerHTML = `
                <strong>${textoCompleto.split('(')[0].trim()}</strong><br>
                <span style="color: ${colorPlan}">${estadoPlan}</span>
            `;
    ...
    function actualizarResumen() {
        if (document.getElementById('resumen_estudiante')) {
            ...
            const hora = horaContactoInput?.value || '';
            document.getElementById('resumen_fecha').textContent = fecha && hora ? `${fecha} ${hora}` : 'No seleccionada';
    ...
    function validarPaso(step) {
        if (step === 1) {
            ...
            if (!horaContactoInput?.value) {
                Swal.fire('Error', 'Debe ingresar la hora de contacto', 'error');
                return false;
            }
    ...
    if (horaContactoInput) horaContactoInput.addEventListener('change', actualizarResumen);
    ...
    formCampos.forEach(campo => {
        if (campo.type !== 'hidden' && !campo.disabled && campo.name !== '_token' && campo.name !== '_method') {
    ...
    document.getElementById('interaccionForm')?.addEventListener('submit', function() {
        ...
        btn.disabled = true;
        btn.innerHTML = '<i class="fas fa-spinner fa-spin me-2"></i> Actualizando interacción...';
    });
