...
<div class="container-fluid px-4">
    ...
    <div class="row g-4">
        ...
        <!-- Columna de preguntas - CON JUSTIFICACIÓN -->
        <div class="col-lg-8">
                ...
                <div class="card-header bg-transparent border-0 pt-4 px-4">
                    <div class="d-flex justify-content-between align-items-center flex-wrap gap-3">
                        <div>
                            ...
                            <p class="text-muted small mt-1">Haz clic en cada pregunta para ver las respuestas y justificación</p>
                        </div>
                        <div class="d-flex gap-2">
                    ...
                    @if(isset($examen->preguntas_lista) && $examen->preguntas_lista->count() > 0)
                        <div class="accordion" id="preguntasAccordion">
                            @foreach($examen->preguntas_lista as $index => $pregunta)
                                @php
                                    // Obtener la pregunta completa para la justificación
                                    $preguntaCompleta = \App\Models\Pregunta::find($pregunta->id);
                                    $justificacion = $preguntaCompleta ? $preguntaCompleta->justificacion : null;
                                ...
                                <div class="accordion-item mb-3 border-0 shadow-sm rounded-4 overflow-hidden">
                                                ...
                                                <div class="flex-grow-1">
                                                    ...
                                                    <div class="d-flex gap-2 flex-wrap">
                                                        ...
                                                        <span class="badge bg-success bg-opacity-10 text-success">
                                                            <i class="fas fa-check-circle me-1"></i>Correcta: {{ $respuestaCorrecta }}
                                                        </span>
                                                        @if($justificacion)
                                                            <span class="badge bg-warning bg-opacity-10 text-warning">
                                                                <i class="fas fa-lightbulb me-1"></i>Con justificación
                                                            </span>
                                                        @endif
                                                    </div>
                                                </div>
                                    ...
                                    <div id="collapse{{ $index }}" class="accordion-collapse collapse" aria-labelledby="heading{{ $index }}" data-bs-parent="#preguntasAccordion">
                                        <div class="accordion-body p-4">
                                            <!-- Enunciado completo -->
                                            <div class="mb-4 p-3 rounded-4" style="background: rgba(102,126,234,0.05);">
                                                <h6 class="fw-semibold mb-2">
                                                    <i class="fas fa-question-circle me-2 text-primary"></i>Enunciado
                                                </h6>
                                                <p class="mb-0">{{ $pregunta->texto_pregunta ?? 'Pregunta sin texto' }}</p>
                                            </div>
                                            
                                            <div class="row g-3">
                                                <!-- Opción A -->
                                                <div class="col-md-6">
                                                    <div class="card border-0 rounded-4 h-100 {{ $respuestaCorrecta == 'A' ? 'border-success border-2' : '' }}" style="background: rgba(102, 126, 234, 0.05);">
                                                        <div class="card-body p-3">
                                                            <div class="d-flex align-items-start gap-3">
                                                                <div class="rounded-circle {{ $respuestaCorrecta == 'A' ? 'bg-success text-white' : 'bg-primary bg-opacity-10 text-primary' }} fw-bold" style="width: 35px; height: 35px; display: flex; align-items: center; justify-content: center; flex-shrink: 0;">
                                                                    A
                                                                </div>
                                                                <div>
                                                                    <strong>Opción A</strong>
                                                                    <p class="mb-0 mt-1">{{ $pregunta->respuesta1 ?? 'N/A' }}</p>
                                                ...
                                                <!-- Opción B -->
                                                <div class="col-md-6">
                                                    <div class="card border-0 rounded-4 h-100 {{ $respuestaCorrecta == 'B' ? 'border-success border-2' : '' }}" style="background: rgba(118, 75, 162, 0.05);">
                                                        <div class="card-body p-3">
                                                            <div class="d-flex align-items-start gap-3">
                                                                <div class="rounded-circle {{ $respuestaCorrecta == 'B' ? 'bg-success text-white' : 'bg-warning bg-opacity-10 text-warning' }} fw-bold" style="width: 35px; height: 35px; display: flex; align-items: center; justify-content: center; flex-shrink: 0;">
                                                                    B
                                                                </div>
                                                                <div>
                                                                    <strong>Opción B</strong>
                                                                    <p class="mb-0 mt-1">{{ $pregunta->respuesta2 ?? 'N/A' }}</p>
                                            ...
                                            <div class="mt-4 p-3 rounded-4" style="background: rgba(245, 158, 11, 0.08); border-left: 4px solid #f59e0b;">
                                                <div class="d-flex align-items-center gap-2 mb-2">
                                                    <i class="fas fa-lightbulb text-warning fa-lg"></i>
                                                    <h6 class="fw-semibold mb-0">Justificación</h6>
                                                </div>
                                                @if($justificacion && !empty($justificacion))
                                                    <p class="mb-0" style="line-height: 1.6;">{{ $justificacion }}</p>
                                                @else
                                                    <p class="mb-0 text-muted">
                                                        <i class="fas fa-info-circle me-1"></i>
                                                        Esta pregunta no tiene justificación registrada.
                                                    </p>
                                                @endif
                                            </div>
                                        </div>
