...
@section('title', 'Editar Examen - SAINS')
        ...
        <div class="d-flex justify-content-between align-items-center flex-wrap gap-3">
            <div>
                <h1 class="display-5 fw-bold" style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); -webkit-background-clip: text; background-clip: text; color: transparent;">
                    <i class="fas fa-edit me-3"></i>Editar Examen #{{ $examen->id }}
                </h1>
                <p class="text-muted">Modifique la configuración y las preguntas del examen</p>
            </div>
...
<div class="row">
    <div class="col-12">
        <div class="card-modern p-4 p-lg-5">
            <form action="{{ route('admin.examenes.update', $examen->id) }}" method="POST" id="examenForm" novalidate>
                @csrf
                @method('PUT')
                    ...
                    <div class="row g-4">
                        <div class="col-md-4">
                            ...
                            <select name="tipo_examen" class="form-select form-select-lg @error('tipo_examen') is-invalid @enderror" required>
                                ...
                                <option value="Materia" {{ old('tipo_examen', $examen->tipo_examen) == 'Materia' ? 'selected' : '' }}>📚 Materia</option>
                                <option value="General del curso" {{ old('tipo_examen', $examen->tipo_examen) == 'General del curso' ? 'selected' : '' }}>🎓 General del curso</option>
                                <option value="Simulación" {{ old('tipo_examen', $examen->tipo_examen) == 'Simulación' ? 'selected' : '' }}>🎯 Simulación</option>
                            </select>
                            @error('tipo_examen')
                        ...
                        <div class="col-md-4">
                            ...
                            <input type="number" name="numero_preguntas" id="numero_preguntas" 
                                   ...
                                   value="{{ old('numero_preguntas', $examen->numero_preguntas) }}" min="1" max="200" required>
                        ...
                        <div class="col-md-4">
                            ...
                            <input type="number" name="tiempo" class="form-control form-control-lg @error('tiempo') is-invalid @enderror" 
                                   value="{{ old('tiempo', $examen->tiempo) }}" min="1" max="180" required>
                            @error('tiempo')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                                            ...
                                            <tr data-area="{{ $pregunta->id_area ?? '' }}" data-id="{{ $pregunta->id }}" class="fila-pregunta">
                                                <td class="text-center">
                                                    <input type="checkbox" name="preguntas_seleccionadas[]" 
                                                           ...
                                                           {{ in_array($pregunta->id, $preguntasSeleccionadas) ? 'checked' : '' }}>
                                                </td>
                ...
                <div class="step-content" data-step="3" style="display: none;">
                    <div class="d-flex align-items-center gap-3 mb-4 pb-2 border-bottom">
                        ...
                        <div>
                            ...
                            <p class="text-muted small mb-0">Revise los datos antes de actualizar el examen</p>
                        </div>
                    </div>
...
<script>
    ...
    document.getElementById('examenForm').addEventListener('submit', function(e) {
        ...
        btn.disabled = true;
        btn.innerHTML = '<i class="fas fa-spinner fa-spin me-2"></i> Actualizando examen...';
    });
