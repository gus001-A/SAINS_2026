...
@section('title', 'Editar Estudiante - SAINS')
        ...
        <div class="d-flex justify-content-between align-items-center flex-wrap gap-3">
            <div>
                <h1 class="display-5 fw-bold" style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); -webkit-background-clip: text; background-clip: text; color: transparent;">
                    <i class="fas fa-user-edit me-3"></i>Editar Estudiante
                </h1>
                <p class="text-muted">Modifique la información del estudiante en el sistema</p>
            </div>
...
<div class="row">
    <div class="col-12">
        <div class="card-modern p-4 p-lg-5">
            <form action="{{ route('admin.estudiantes.update', $user->id) }}" method="POST" id="estudianteForm" novalidate>
                @csrf
                @method('PUT')
                    ...
                    <div class="row g-4">
                        <div class="col-md-4">
                            ...
                            <input type="text" name="nombre" class="form-control form-control-lg @error('nombre') is-invalid @enderror" 
                                   value="{{ old('nombre', $estudianteData->nombre) }}" placeholder="Ej: Juan" required>
                            @error('nombre')
                        ...
                        <div class="col-md-4">
                            ...
                            <input type="text" name="paterno" class="form-control form-control-lg @error('paterno') is-invalid @enderror" 
                                   value="{{ old('paterno', $estudianteData->paterno) }}" placeholder="Ej: Pérez" required>
                            @error('paterno')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="col-md-4">
                            ...
                            <input type="text" name="materno" class="form-control form-control-lg @error('materno') is-invalid @enderror" 
                                   value="{{ old('materno', $estudianteData->materno) }}" placeholder="Ej: García">
                            @error('materno')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="col-md-4">
                            ...
                            <input type="date" name="fecha_nacimiento" id="fecha_nacimiento" 
                                ...
                                value="{{ old('fecha_nacimiento', $estudianteData->fecha_nacimiento ? \Carbon\Carbon::parse($estudianteData->fecha_nacimiento)->format('Y-m-d') : '') }}" 
                        ...
                        <div class="col-md-4">
                            ...
                            <select name="sexo" class="form-select form-select-lg @error('sexo') is-invalid @enderror" required>
                                ...
                                <option value="M" {{ old('sexo', $estudianteData->sexo) == 'M' ? 'selected' : '' }}>Masculino</option>
                                <option value="F" {{ old('sexo', $estudianteData->sexo) == 'F' ? 'selected' : '' }}>Femenino</option>
                            </select>
                            @error('sexo')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="col-md-4">
                            ...
                            <input type="tel" name="telefono" id="telefono" class="form-control form-control-lg @error('telefono') is-invalid @enderror" 
                                   value="{{ old('telefono', $estudianteData->telefono) }}" placeholder="7771234567" maxlength="10" 
                        ...
                        <div class="col-md-4">
                            ...
                            <input type="tel" name="telefono_casa" id="telefono_casa" class="form-control form-control-lg @error('telefono_casa') is-invalid @enderror" 
                                   value="{{ old('telefono_casa', $estudianteData->telefono_casa) }}" placeholder="7771234567" maxlength="10" 
                ...
                <div class="step-content" data-step="2" style="display: none;">
                    ...
                    <div class="row g-4">
                        ...
                        <div class="col-md-4">
                            ...
                            <select id="estado_prepa" class="form-select form-select-lg">
                                ...
                                @foreach($estadosPrepa as $estado)
                                    <option value="{{ $estado }}" {{ $estudianteData->escuela_procedencia_estado == $estado ? 'selected' : '' }}>{{ $estado }}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div class="col-md-4">
                            ...
                            <select id="estado_uni" class="form-select form-select-lg">
                                ...
                                @foreach($estadosUniversidad as $estado)
                                    <option value="{{ $estado }}" {{ $estudianteData->universidad_interes_estado == $estado ? 'selected' : '' }}>{{ $estado }}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div class="col-md-6">
                            ...
                            @if($estudianteData->cupon)
                                <div class="alert alert-success mb-0">
                                    <div class="d-flex align-items-center gap-3">
                                        <i class="fas fa-check-circle fa-2x"></i>
                                        <div class="flex-grow-1">
                                            <strong class="d-block">Cupón aplicado</strong>
                                            <span class="font-monospace fs-5">{{ $estudianteData->cupon }}</span>
                                            <small class="d-block text-muted mt-1">El cupón ya fue utilizado y no puede ser modificado</small>
                                        </div>
                                    </div>
                                </div>
                                <input type="hidden" name="cupon_existente" value="{{ $estudianteData->cupon }}">
                            @else
                                ...
                                <div id="cuponMensaje" class="mt-2">
                                    <small class="text-muted"><i class="fas fa-info-circle me-1"></i>Seleccione un cupón de descuento si está disponible</small>
                                </div>
                                @error('cupon_id')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="form-check form-switch mt-2">
                                <input type="checkbox" name="plan_activo" class="form-check-input" id="plan_activo" style="width: 50px; height: 25px;" 
                                       value="1" {{ old('plan_activo', $estudianteData->plan_activo) ? 'checked' : '' }}>
                                <label class="form-check-label ms-2 fw-semibold" for="plan_activo" id="planActivoLabel">
                                    {{ old('plan_activo', $estudianteData->plan_activo) ? 'Activo' : 'Inactivo' }}
                                </label>
                            </div>
                ...
                <div class="step-content" data-step="3" style="display: none;">
                    ...
                    <div class="row g-4">
                        <div class="col-md-6">
                            ...
                            <div class="input-group input-group-lg">
                                ...
                                <input type="email" name="email" class="form-control @error('email') is-invalid @enderror" 
                                       value="{{ old('email', $estudianteData->correo) }}" placeholder="estudiante@ejemplo.com" required>
                                @error('email')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                        ...
                        <div class="col-md-6">
                            <label class="form-label fw-semibold">
                                <i class="fas fa-lock text-primary me-1"></i> Nueva Contraseña
                            </label>
                            <div class="input-group input-group-lg">
                            ...
                            <small class="text-muted"><i class="fas fa-info-circle me-1"></i>Mínimo 6 caracteres. Déjelo en blanco para mantener la contraseña actual</small>
                        </div>
                        
                        <div class="col-md-6">
                            <label class="form-label fw-semibold">
                                <i class="fas fa-check-circle text-primary me-1"></i> Confirmar Contraseña
                            </label>
                            <div class="input-group input-group-lg">
                                <span class="input-group-text bg-transparent"><i class="fas fa-check-circle text-primary"></i></span>
                                <input type="password" name="password_confirmation" class="form-control" 
                                       id="password_confirmation" placeholder="Repite la nueva contraseña">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6">
                                ...
                                <div class="confirmation-item">
                                    ...
                                    <span id="confirm_password_change"></span>
                                </div>
                            </div>
                        </div>
                    </div>
...
<script>
    ...
    // Datos actuales para los filtros
    const escuelaActualId = "{{ $estudianteData->escuela_procedencia }}";
    const escuelaActualEstado = "{{ $estudianteData->escuela_procedencia_estado }}";
    const escuelaActualMunicipio = "{{ $estudianteData->escuela_procedencia_municipio }}";
    const escuelaActualLocalidad = "{{ $estudianteData->escuela_procedencia_localidad }}";
    
    const universidadActualId = "{{ $estudianteData->universidad_interes }}";
    const universidadActualEstado = "{{ $estudianteData->universidad_interes_estado }}";
    const universidadActualMunicipio = "{{ $estudianteData->universidad_interes_municipio }}";
    const universidadActualLocalidad = "{{ $estudianteData->universidad_interes_localidad }}";
    
    // Verificar si ya tiene cupón
    const tieneCupon = {{ $estudianteData->cupon ? 'true' : 'false' }};
    
    // Variables de control
    let cargaInicialUniversidad = false;
    let cargaInicialEscuela = false;
    ...
    async function cargarDatosInicialesEscuela() {
        if (!escuelaActualEstado || !escuelaActualMunicipio) return;
        
        console.log('Cargando datos iniciales de escuela...');
        
        // 1. Cargar estado
        estadoPrepa.value = escuelaActualEstado;
        
        // 2. Esperar y cargar municipios
        await new Promise(resolve => setTimeout(resolve, 500));
        
        // Disparar cambio de estado para cargar municipios
        estadoPrepa.dispatchEvent(new Event('change'));
        
        // 3. Esperar a que se carguen los municipios
        await new Promise(resolve => setTimeout(resolve, 1000));
        
        // 4. Seleccionar municipio
        municipioPrepa.value = escuelaActualMunicipio;
        
        // 5. Disparar cambio de municipio para cargar localidades
        municipioPrepa.dispatchEvent(new Event('change'));
        
        // 6. Esperar a que se carguen las localidades
        await new Promise(resolve => setTimeout(resolve, 1000));
        
        // 7. Seleccionar localidad si existe
        if (escuelaActualLocalidad && escuelaActualLocalidad !== '') {
            localidadPrepa.value = escuelaActualLocalidad;
            localidadPrepa.dispatchEvent(new Event('change'));
            await new Promise(resolve => setTimeout(resolve, 1000));
        }
        
        // 8. Seleccionar la preparatoria específica
        await new Promise(resolve => setTimeout(resolve, 500));
        for (let i = 0; i < escuelaSelect.options.length; i++) {
            if (escuelaSelect.options[i].value == escuelaActualId) {
                escuelaSelect.selectedIndex = i;
                break;
            }
        }
        
        cargaInicialEscuela = true;
        console.log('Escuela cargada correctamente');
    }
    ...
    localidadPrepa.addEventListener('change', function() {
        cargarPreparatorias();
    });
    ...
    function cargarPreparatorias() {
        ...
        fetch(url)
            ...
            .then(data => {
                ...
                // Si es carga inicial, seleccionar la escuela actual
                if (!cargaInicialEscuela && escuelaActualId) {
                    for (let i = 0; i < escuelaSelect.options.length; i++) {
                        if (escuelaSelect.options[i].value == escuelaActualId) {
                            escuelaSelect.selectedIndex = i;
                            break;
    ...
    // ========== FILTROS PARA UNIVERSIDAD (VERSIÓN CORREGIDA) ==========
    ...
    async function cargarDatosInicialesUniversidad() {
        if (!universidadActualEstado || !universidadActualMunicipio) {
            console.log('No hay datos de universidad para cargar');
            return;
        }
        
        console.log('Cargando datos iniciales de universidad...');
        console.log('Estado:', universidadActualEstado);
        console.log('Municipio:', universidadActualMunicipio);
        console.log('Localidad:', universidadActualLocalidad);
        console.log('ID Universidad:', universidadActualId);
        
        try {
            // 1. Establecer estado
            estadoUni.value = universidadActualEstado;
            
            // 2. Disparar cambio de estado para cargar municipios
            estadoUni.dispatchEvent(new Event('change'));
            
            // 3. Esperar a que se carguen los municipios
            await new Promise(resolve => setTimeout(resolve, 1500));
            
            // 4. Establecer municipio
            if (municipioUni.querySelector(`option[value="${universidadActualMunicipio}"]`)) {
                municipioUni.value = universidadActualMunicipio;
            } else {
                console.warn('Municipio no encontrado:', universidadActualMunicipio);
            }
            
            // 5. Disparar cambio de municipio para cargar localidades
            municipioUni.dispatchEvent(new Event('change'));
            
            // 6. Esperar a que se carguen las localidades
            await new Promise(resolve => setTimeout(resolve, 1500));
            
            // 7. Establecer localidad si existe
            if (universidadActualLocalidad && universidadActualLocalidad !== '') {
                if (localidadUni.querySelector(`option[value="${universidadActualLocalidad}"]`)) {
                    localidadUni.value = universidadActualLocalidad;
                    localidadUni.dispatchEvent(new Event('change'));
                    await new Promise(resolve => setTimeout(resolve, 1500));
                } else {
                    console.warn('Localidad no encontrada:', universidadActualLocalidad);
                    // Aún así cargar universidades
                    await cargarUniversidadesAsync();
                }
            } else {
                // Si no hay localidad, cargar universidades directamente
                await cargarUniversidadesAsync();
            }
            
            // 8. Seleccionar la universidad específica
            await new Promise(resolve => setTimeout(resolve, 1000));
            
            let universidadEncontrada = false;
            for (let i = 0; i < universidadSelect.options.length; i++) {
                if (universidadSelect.options[i].value == universidadActualId) {
                    universidadSelect.selectedIndex = i;
                    universidadEncontrada = true;
                    console.log('Universidad encontrada y seleccionada:', universidadSelect.options[i].text);
                    break;
                }
            }
            
            if (!universidadEncontrada) {
                console.warn('No se encontró la universidad con ID:', universidadActualId);
                console.log('Opciones disponibles:', Array.from(universidadSelect.options).map(opt => ({value: opt.value, text: opt.text})));
            }
            
            cargaInicialUniversidad = true;
            console.log('Universidad cargada correctamente');
        } catch (error) {
            console.error('Error al cargar datos iniciales de universidad:', error);
        }
    }
    
    async function cargarUniversidadesAsync() {
        ...
        return new Promise((resolve) => {
            ...
            if (localidad && localidad !== '' && localidad !== 'Todas las localidades') {
                url += `&localidad=${encodeURIComponent(localidad)}`;
            }
            
            console.log('Cargando universidades desde URL:', url);
            
            fetch(url)
                .then(response => response.json())
                .then(data => {
                    console.log('Universidades recibidas:', data.length);
    ...
    localidadUni.addEventListener('change', function() {
        cargarUniversidades();
    });
    ...
    function cargarUniversidades() {
        ...
        fetch(url)
            ...
            .then(data => {
                ...
                // Si es carga inicial, seleccionar la universidad actual
                if (!cargaInicialUniversidad && universidadActualId) {
                    for (let i = 0; i < universidadSelect.options.length; i++) {
                        if (universidadSelect.options[i].value == universidadActualId) {
                            universidadSelect.selectedIndex = i;
                            console.log('Universidad seleccionada en carga inicial:', universidadSelect.options[i].text);
                            break;
                        }
                    }
                }
    ...
    if (!tieneCupon) {
        ...
        if (validarCuponBtn) {
            validarCuponBtn.addEventListener('click', function() {
                ...
                const selectedOption = cuponSelect.options[cuponSelect.selectedIndex];
                const cuponCodigo = selectedOption.textContent.split(' - ')[0];
    ...
    function actualizarConfirmacion() {
        ...
        let cuponTexto = '';
        if (tieneCupon) {
            cuponTexto = '{{ $estudianteData->cupon }} (ya aplicado - no modificable)';
        } else {
            const cuponSelect2 = document.getElementById('cupon_id');
            cuponTexto = cuponSelect2?.options[cuponSelect2.selectedIndex]?.text.split(' - ')[0] || 'Sin cupón';
            if (cuponTexto && cuponTexto !== 'Sin cupón') {
                cuponTexto += ' (nuevo)';
            }
        }
        
        document.getElementById('confirm_nombre_completo').textContent = `${nombre} ${paterno} ${materno}`.trim();
        document.getElementById('confirm_fecha_nacimiento').textContent = fechaNacimiento ? new Date(fechaNacimiento).toLocaleDateString('es-MX') : 'No especificada';
        ...
        document.getElementById('confirm_password_change').textContent = password ? 'Sí, se cambiará la contraseña' : 'No se modificará la contraseña';
    }
    ...
    function validarPaso(step) {
        ...
        if (step === 3) {
            ...
            if (password.length > 0 && password.length < 6) { Swal.fire('Error', 'La contraseña debe tener al menos 6 caracteres', 'error'); return false; }
    ...
    // Inicializar - Cargar datos actuales y mostrar paso 1
    // Esperamos a que el DOM esté completamente cargado
    document.addEventListener('DOMContentLoaded', function() {
        console.log('DOM cargado, iniciando carga de datos...');
        setTimeout(() => {
            cargarDatosInicialesEscuela();
            cargarDatosInicialesUniversidad();
        }, 500);
    });
